...
export default function Dashboard({ adminProtected }: { adminProtected: boolean }) {
  ...
  useEffect(() => {
    ...
    const loadLeads = () => fetch("/api/leads")
      ...
      .then((data: { leads?: Lead[] }) => { if (active) setLeads(data.leads ?? []); });

    Promise.all([
      loadLeads(),
      fetch("/api/pipeline").then((response) => response.json()).then((data: { stages?: PipelineStage[] }) => {
        if (active && data.stages?.length) setStages(data.stages);
      }),
    ])
      .catch(() => notify("Не удалось загрузить лидов"))
      .finally(() => { if (active) setLoadingLeads(false); });

    // Without this the dashboard shows the state it was opened with and misses every new lead.
    const refresh = () => { if (document.visibilityState === "visible") void loadLeads().catch(() => undefined); };
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        <div className="content">
          {!adminProtected && <div className="security-banner"><strong>Кабинет открыт без пароля</strong><span>Задайте <code>ADMIN_PASSWORD</code> в <code>.env</code> и перезапустите сервис — иначе лиды и переписки доступны каждому, кто знает адрес.</span></div>}
...
function Inbox({ leads, stages, notify, setView }: { leads: Lead[]; stages: PipelineStage[]; notify: (message: string) => void; setView: (view: View) => void }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    const load = () => fetch(`/api/leads/${encodeURIComponent(activeLeadId)}/messages`)
      .then((response) => response.json())
      .then((data) => { if (active) setMessages(data.messages ?? []); });

    load().catch(() => { if (active) setMessages([]); });
    // The reply from the client has to land in the open dialog on its own.
...
function Broadcasts({ activeLanguage, setActiveLanguage, drafts, setDrafts, editorRef, insertFormatting, notify, leads, stages }: {
  ...
  async function sendBroadcast() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) notify(result.error || `Не отправлено: ${result.failed ?? 0}`);
      else if (result.failed) notify(`Отправлено: ${result.sent}, не доставлено: ${result.failed}${result.error ? ` — ${result.error}` : ""}`);
      else notify(`Отправлено: ${result.sent}`);
    } finally {
      setSending(false);
    }
  }
